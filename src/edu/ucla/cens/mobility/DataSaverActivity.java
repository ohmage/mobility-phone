...
//        	Cursor c = MobilityInterface.getMobilityCursor(DataSaverActivity.this, 0L);
		MobilityDbAdapter mdb = new MobilityDbAdapter(DataSaverActivity.this);
        	...
        	int ki = 0;
        	for (String s : new String[] { MobilityInterface.KEY_ROWID, MobilityInterface.KEY_MODE, MobilityInterface.KEY_SPEED, MobilityInterface.KEY_STATUS, MobilityInterface.KEY_LOC_TIMESTAMP, MobilityInterface.KEY_ACCURACY, MobilityInterface.KEY_PROVIDER, MobilityInterface.KEY_WIFIDATA, MobilityInterface.KEY_ACCELDATA, MobilityInterface.KEY_TIME, MobilityInterface.KEY_TIMEZONE, MobilityInterface.KEY_LATITUDE, MobilityInterface.KEY_LONGITUDE })
        	{
        		indexMap.put(s, ki++);
        	}
...
//    				}
    				BufferedWriter outWrite = new BufferedWriter(
    					      new FileWriter(Environment.getExternalStorageDirectory() + "/" +  filename + "_" + index++ + ".txt"));
//    				Log.i(TAG, "Attempting to write a batch with " + String.valueOf(mpoints.size()) + " mobility points.");
    				...
    				for (int i = 0; i < mpoints.size(); i++) 
    				{
    					...
    					try {
    						...
    						//Log.d(TAG, Float.parseFloat(c.getString(c.getColumnIndex(MobilityInterface.KEY_SPEED))) + " is the speed");
    						...
    						if (! locationStatus.equals("unavailable")) {
    							...
    							if (!Double.isNaN(Double.parseDouble(row.latitudeValue)))//c.getString(c.getColumnIndex(MobilityInterface.KEY_LATITUDE)))))
    								locationJson.put("latitude", Double.parseDouble(row.latitudeValue));//c.getString(c.getColumnIndex(MobilityInterface.KEY_LATITUDE))));
    							else
    							...
    							if (!Double.isNaN(Double.parseDouble(row.longitudeValue)))//c.getString(c.getColumnIndex(MobilityInterface.KEY_LONGITUDE)))))
    								locationJson.put("longitude", Double.parseDouble(row.longitudeValue));//c.getString(c.getColumnIndex(MobilityInterface.KEY_LONGITUDE))));
    							else
    							...
    							locationJson.put("timestamp", dateFormat.format(new Date(Long.parseLong(row.locTimeValue))));//c.getString(c.getColumnIndex(MobilityInterface.KEY_LOC_TIMESTAMP))))));
		...
		protected String doInBackground(String... params)
		{
			synchronized (rescueLock)
			{
				try
				{
					...
					if (skipped > 0)
						return "...finished! Data stored on sd card. " + skipped + " records were skipped.";
					else
	...
	public void rescueData(String filename) throws IOException
	{
		...
		if (c != null && c.getCount() > 0)
		{
			...
			while (remainingCount > 0)
			{
				...
				BufferedWriter outWrite = new BufferedWriter(new FileWriter(Environment.getExternalStorageDirectory() + "/" + filename + "_" + index++ + ".txt"));
				Log.i(TAG, "Attempting to write a batch with " + String.valueOf(limit) + " mobility points.");
				...
				for (int i = 0; i < limit; i++)
				{
					...
					try
					{
						...
						Log.d(TAG, Float.parseFloat(c.getString(c.getColumnIndex(MobilityInterface.KEY_SPEED))) + " is the speed");
						if (!Float.isNaN(Float.parseFloat(c.getString(c.getColumnIndex(MobilityInterface.KEY_SPEED)))))
						...
						{
							JSONObject locationJson = new JSONObject();
							if (!Double.isNaN(Double.parseDouble(c.getString(c.getColumnIndex(MobilityInterface.KEY_LATITUDE)))))
								locationJson.put("latitude", Double.parseDouble(c.getString(c.getColumnIndex(MobilityInterface.KEY_LATITUDE))));
							else
								locationJson.put("latitude", null);

							if (!Double.isNaN(Double.parseDouble(c.getString(c.getColumnIndex(MobilityInterface.KEY_LONGITUDE)))))
