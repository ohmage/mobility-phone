...
//			 }
	...
	public static void start(Context context)
	{
		...
		if (c.moveToFirst())
		{
			do
			{
				...
				Log.d(TAG,
						start.getHour() + ":" + start.getMinute() + " until "
								+ end.getHour() + ":" + end.getMinute()
								+ " is blackout and now is " + now.getHour()
								+ now.getMinute());
	...
	public static void setNotification(Context context, String status,
			String message)
	{
	...
	public static void startMobility(Context context)
	{
		...
		setNotification(context, STATUS_PENDING,
		...
		SharedPreferences settings = context.getSharedPreferences(MOBILITY,
		...
		Log.d(TAG, "Starting transport mode service with sampleRate: "
				+ sampleRate);
	...
	public static void stopMobility(Context context, boolean blackout)
	{
		...
		if (mgr != null)
		{
			...
			startPI = PendingIntent.getBroadcast(context, 0, i,
		...
		// if (nm != null)
		{
			if (blackout)
			{
				setNotification(context, STATUS_BLACKOUT,
						"Mobility paused during blackout time");
			}
			else
			{
